...
const useInAppFunnel = (): StatusQuery => {
  ...
  useEffect(() => {
    ...
    if (Object.values(state.dateRange).length !== 0) {
      apiDashboard
        .get(`/campaign-mgmt-api/analytics/campaign/aggregated?startDate=${fromDate}&endDate=${toDate}&campaignType=INAPP`)
        .then((response) => {
          ...
          dispatch({
            type: CAMPAIGN_ANALYTICS_ACTIONS.SET_IN_APP_CHART_DATA,
