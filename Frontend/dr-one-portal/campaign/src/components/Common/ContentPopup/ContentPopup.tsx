...
import * as S from "./ContentPopup.styles";
        ...
        <div className="modal-footer align-right cc-global-buttons">
          ...
          <Button className="button-xs" variant="contained" color="primary" type="submit" disabled={Object.keys(selectedContent).length === 0} startIcon={<ArrowForwardOutlinedIcon fontSize="small" />}
            onClick={(e) => {
              ...
              modifiedPayload['creative'][props.imageContentType === 'NOTIFICATION' ? 'notificationImageContent' : props.imageContentType === 'RICHNOTIFICATION_BIG' ? 'richNotificationImageContent' : props.imageContentType === 'FULL' ? 'fullImageContent' : props.imageContentType === 'MAIN' ? 'mainImageContent' : props.imageContentType === 'GIF' ? 'gifImageContent' :
                'videoContent'] = selectedContent;
              if (props.contentType === 'videocontents' || props.contentType === 'gifcontents') {
                modifiedPayload['creative'][props.imageContentType === 'GIF' ? 'videoContent' : 'gifImageContent'] = {};
                if (props.imageContentType === 'GIF') {
                  getFileFromUrl(selectedContent?.videoFileUrl, String(new Date().getTime()) + '.gif').then(res => {
                    modifiedPayload['creative']['gifDuration'] = res;
                    dispatch({
                      type: CAMPAIGN_ACTIONS.MODIFY_CAMPAIGN_PAYLOAD,
                      payload: {
                        campaignPayload: modifiedPayload, currentPageName: props.section,
