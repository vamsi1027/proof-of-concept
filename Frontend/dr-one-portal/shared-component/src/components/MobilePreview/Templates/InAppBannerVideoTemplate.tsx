...
export function InAppBannerVideoTemplate(props: MobilePreviewProps) {
    ...
    return (
        <div className={props.cpType === 'ENGAGEMENT' ? 'pr-wrapper-templete inApp-template popup-banner-video engagement-popup-video' : 'pr-wrapper-templete inApp-template popup-banner-video'}>
            <div className="popup-banner-video-inner">
                ...
                <div className="banner-video">
                    <div className="inapp-full-img-wrapper">
                        {(props.message.banner && ((props.message?.video?.split('.').pop() !== 'gif' && !isSelectedVideoPlaying)
                            || (props.message?.video?.split('.').pop() === 'gif' && !isGifPlaying))) && (
                                <img src={props.message.banner} alt="InAppBannerVideoTemplate" />
                            )}
                    </div>
                    <div className="inapp-full-video-wrapper">
                        ...
                        {(props.message?.video && isSelectedVideoPlaying && props.message?.video?.split('.').pop() !== 'gif') && <video controls controlsList="nodownload nofullscreen noremoteplayback" preload="metadata" onClick={playAndPause} onEnded={videoEnded} onCanPlay={isVideoLoaded} ref={playerRef} hidden={!videoCanPlay}>
                            <source src={props.message.video} type="video/mp4" />
                        </video>}
                        {(props.message?.video && !isSelectedVideoPlaying && props.message?.video?.split('.').pop() !== 'gif' && props?.message?.banner) &&
                            <PlayArrowIcon onClick={(e) => playSelectedVideo()} className="play-video" />}

                        {(props.message?.video && !isGifPlaying && props.message?.video?.split('.').pop() === 'gif' && props?.message?.banner
                        ) && <PlayArrowIcon onClick={(e) => toggleGifImage()} className="play-video" />}
                    </div>
